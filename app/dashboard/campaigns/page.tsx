...
import { Plus, Megaphone, Users, TrendingUp, Calendar, Home, ArrowRight, Eye } from "lucide-react"
...
export default async function CampaignsOverviewPage() {
  ...
  // Calculate stats
  const totalViews = campaignsList.reduce((sum, c) => sum + (c.views || 0), 0)
  const totalLeads = campaignsList.reduce((sum, c) => sum + (c.leads || 0), 0)
  const activeCampaigns = campaignsList.filter((c) => c.status === "active").length
  const thisMonthCampaigns = campaignsList.filter(
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{thisMonthCampaigns}</div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Campaign views</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <Link key={campaign.id} href={`/dashboard/campaigns/${campaign.id}`}>
                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors cursor-pointer">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <h3 className="font-semibold">{campaign.property_address || "Untitled Campaign"}</h3>
                            <p className="text-sm text-muted-foreground">
                              {campaign.bedrooms && campaign.bathrooms && campaign.price
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="text-right">
                            <p className="text-xs text-muted-foreground">
                              Updated {new Date(campaign.updated_at || campaign.created_at).toLocaleDateString()}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {campaign.views || 0} views • {campaign.leads || 0} leads
                            </p>
                          </div>
                          <Badge
                            variant={
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="mt-2 text-sm text-muted-foreground text-center max-w-sm">
                  Get started by creating your first listing campaign. Generate professional marketing materials in
                  minutes.
                </p>
                <CampaignWizardTrigger>
